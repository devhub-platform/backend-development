    ...
    public function run(): void
    {
        ...
        foreach ($questions as $i => $data) {
            ...
            $answerContents = [
                ...
                'Eager loading uses the with() method to load relationships in a single query. Lazy loading loads them on demand which causes N+1 problem.',
                'Use the ShouldQueue interface on your Mailable class and configure your QUEUE_CONNECTION in .env to use database or redis.',
            ];

            $acceptedCount = 0;
            foreach ($answerContents as $j => $content) {
                $answerer  = $users[($i + $j + 1) % $users->count()];
                $isAccepted = $j === 0 && $i % 2 === 0; // accept first answer on even questions
                ...
                foreach ($users->take(3) as $voter) {
                    ...
                    AnswerVote::firstOrCreate(
                        ['answer_id' => $answer->id, 'user_id' => $voter->id],
            ...
            // Mark resolved if any accepted answers
            if ($acceptedCount > 0) {
