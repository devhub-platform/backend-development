...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('posts', function (Blueprint $table) {
            if (!$this->indexExists('posts', 'posts_status_created_at_index')) {
                $table->index(['status', 'created_at']);
            }

            if (!$this->indexExists('posts', 'posts_user_id_status_index')) {
                $table->index(['user_id', 'status']);
            }

            if (!$this->indexExists('posts', 'posts_views_created_at_index')) {
                $table->index(['views', 'created_at']);
            }
        });

        Schema::table('followers', function (Blueprint $table) {
            if (!$this->indexExists('followers', 'followers_follower_id_index')) {
        ...
        Schema::table('post_tags', function (Blueprint $table) {
            if (!$this->indexExists('post_tags', 'post_tags_tag_id_index')) {
                $table->index('tag_id');
            }

            if (!$this->indexExists('post_tags', 'post_tags_post_id_index')) {
                $table->index('post_id');
            }
        });

        Schema::table('post_views', function (Blueprint $table) {
            if (!$this->indexExists('post_views', 'post_views_user_id_viewed_at_index')) {
